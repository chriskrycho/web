...
pub fn serve(site_dir: &Path) -> Result<(), Error> {
   ...
   let (rebuild_tx, _) = broadcast::channel(1);
   ...
   let rebuild_handle = rt.spawn(rebuild(site_dir, config, md, change_tx, rebuild_tx));
   ...
   loop {
      match change.recv().await {
         Ok(Change { paths }) => {
            ...
            rebuild_tx.send(rebuild).map(|_| ()).map_err(Error::from)?;
         }
...
async fn serve_in(path: PathBuf, state: Sender<Rebuild>) -> Result<(), Error> {
...
async fn websocket(socket: WebSocket, state: Sender<Rebuild>) {
   let (mut ws_tx, mut ws_rx) = socket.split();

   let mut rebuild_rx = state.subscribe();
   ...
   fn drop(&mut self) {
      trace!(
         ...
         match self {
            ...
            Rebuild::Failure { cause } => format!("Failure({cause:?})"),
         }
      );
   }
}
